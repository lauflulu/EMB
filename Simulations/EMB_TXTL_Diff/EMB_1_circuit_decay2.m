function [t,outcome]= EMB_1_circuit_decay2(a,b)
...
[t,y]=ode23s(@EMB_1_circuit_decay2_function,tspan,x0,options);
...
function ydot = EMB_1_circuit_decay2_function(t, y, flags)

global alphamax aLacI aRFP aTetR aYFP tRNA tmat1 tmat2 t1 t2 KdLacI nLacI KdTetR nTetR kon koff kdeg

dy(1,1)=-kon*y(1,1)*y(2,1)+koff*y(3,1); % IPTG

if t < t1; 
    dy(2,1)=alphamax*aLacI*(1-exp(-t/tRNA))*(1./(1+(y(6,1)./KdTetR).^nTetR))-kdeg-kon*y(1,1)*y(2,1)+koff*y(3,1); % LacI
    dy(4,1)=alphamax*aRFP*(1-exp(-t/tRNA))*(1./(1+(y(6,1)./KdTetR).^nTetR))-kdeg-y(4,1)/tmat1; % RFP
    ...
    dy(2,1)=alphamax*aLacI*(1-exp(-t/tRNA))*(1./(1+(y(6,1)./KdTetR).^nTetR))*exp(-(t-t1)/t2)-kdeg-kon*y(1,1)*y(2,1)+koff*y(3,1);
    dy(4,1)=alphamax*aRFP*(1-exp(-t/tRNA))*(1./(1+(y(6,1)./KdTetR).^nTetR))*exp(-(t-t1)/t2)-kdeg-y(4,1)/tmat1;
...
dy(3,1)=kon*y(1,1)*y(2,1)-koff*y(3,1)-kdeg; % IPTG-LacI

dy(5,1)=y(4,1)/tmat1-kdeg; % RFPmat
