function [t,outcome]= EMB_2_circuit_decay(a)

global  nspecies alphamax aLacI aRFP aTetR aYFP tRNA tmat1 tmat2 t1 t2 KdLacI nLacI KdTetR nTetR kon koff kdeg Kdeg
...
[t,y]=ode23s(@EMB_2_circuit_decay_function,tspan,x0,options);
...
function ydot = EMB_2_circuit_decay_function(t, y, flags)

global alphamax aLacI aRFP aTetR aYFP tRNA tmat1 tmat2 t1 t2 KdLacI nLacI KdTetR nTetR kon koff kdeg Kdeg

dy(1,1)=-kon*y(1,1)*y(2,1)+koff*y(3,1); % IPTG

if t < t1;
    dy(2,1)=alphamax*aLacI*(1-exp(-t/tRNA))*(1./(1+(y(6,1)./KdTetR).^nTetR))-kon*y(1,1)*y(2,1)+koff*y(3,1); % LacI
    ...
    dy(2,1)=alphamax*aLacI*(1-exp(-t/tRNA))*(1./(1+(y(6,1)./KdTetR).^nTetR))*exp(-(t-t1)/t2)-kon*y(1,1)*y(2,1)+koff*y(3,1);
