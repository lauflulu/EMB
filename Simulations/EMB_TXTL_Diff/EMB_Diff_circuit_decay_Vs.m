function [t,outcome]= EMB_Diff_circuit_decay_Vs(a)

global  nspecies ncompartment ... 
    D P V Vs l_s l_ts ls_s ls_ts A As theta ...
...
V = 1.2e-12*1e18; % um3 average volume of compartments, 1 nL
%l = 150.3e-6; % m, average length of compartments
l_s = 2*(4*V./(3*pi)).^(1./3); % um, average length of compartments assuming
%sphere
Vs = a*1e18; % given in m3, changed to um3
ls_s = 2*(4*Vs./(3*pi)).^(1./3); % um, average length of sender compartment
% assuming sphere
theta = 42*pi/180; % ?, interface angle

A = Bilayer_area(l_s, l_s, theta);
l_ts = 2.*(((4*V./(3*pi))+(sqrt(A./pi)).^3).^(1/3)-sqrt(A./pi)); % um, 
% average length of compartments, assuming truncated sphere

As = Bilayer_area(l_s, ls_s, theta);
ls_ts = 2.*(((4*Vs./(3*pi))+(sqrt(As./pi)).^3).^(1/3)-sqrt(As./pi)); % um, 
% average length of compartments, assuming truncated sphere
...
kon = 7.2; % u/M/min, on-rate of IPTG binding LacI, from Xu et al.
...
function ydot = EMB_Diff_circuit_decay_Vs_function(t, y, flags)

global  nspecies ncompartment D P A l_ts V As ls_ts Vs ...
    alphamax aLacI aRFP aTetR aYFP tRNA tmat1 tmat2 t1 t2 ... 
    KdLacI nLacI KdTetR nTetR kon koff kdeg Kdeg
